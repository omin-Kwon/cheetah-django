...
class IdDuplicationCheck(APIView): #회원가입시 아이디 중복 체크
    def get(self, request):
        ...
        if User.objects.filter(username = username).exists():
            return Response({'detail': '중복된 아이디입니다.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get(self, request):
        try:
            ...
            if identification_result == True:
                return Response({'message':'OK', 'identification_result':identification_result})
            if identification_result == False:
                return Response({'message':'전화번호 인증에 실패하였습니다.'}, status=status.HTTP_401_UNAUTHORIZED)
